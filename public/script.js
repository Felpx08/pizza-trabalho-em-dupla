...
function sair() { // Função para sair da aplicação
  TOKEN = '';
  USUARIO_LOGADO = null;
...
function abrir(id)  { document.getElementById(id).classList.add('open'); } // Função para abrir um modal
function fechar(id) { document.getElementById(id).classList.remove('open'); } // Função para fechar um modal
...
async function api(method, url, body) { // Função para fazer requisições à API
  const opts = {
    method,
    headers: {
...
function aplicarPerfil(usuario) { // função pra verificar o cargo do usuario, se é cliente ou ou administrador
  ...
  const perfil  = usuario.perfil; // Obtém o perfil do usuário
  const isAdmin = perfil === 'Administrador';
  const isGar   = perfil === 'Garcom';

  function show(id, visible, type = 'flex') { // Função para mostrar ou esconder elementos
    const el = document.getElementById(id);
    if (el) el.style.display = visible ? type : 'none';
  }

  function showEl(el, visible, type = 'flex') { // Função para mostrar ou esconder elementos
    if (el) el.style.display = visible ? type : 'none';
  }
...
async function carregarMesas(mesaFiltro = null) { // Função para carregar as mesas
  ...
  try { 
    ...
    if (!pedidosFiltrados.length) { // Se não houver pedidos filtrados
      grid.innerHTML = `
    ...
    const porMesa = {}; // Agrupa os pedidos por mesa
    ...
    grid.innerHTML = Object.entries(porMesa).map(([mesa, peds]) => {  // Mapeia os pedidos por mesa
  ...
  } catch (e) { // Trata erros na carga das mesas
...
function addItemMesa() { // Função para adicionar um item à mesa
...
function recalcMesa() {   // Função para recalcular o total da mesa
  let sub = 0;
...
async function confirmarFechamento() { // Função para confirmar o fechamento da mesa
...
function ir(pg, btn) { // Função para navegar entre as seções
...
async function carregarDashboard() { // mostra a tela principal do administrador, dinheiro, pedido e a quantidade de clientes 
  ...
  } catch (e) { toast('Erro dashboard: ' + e.message, 'err'); } // Trata erros na carga do dashboard
}
...
async function carregarPizzas() { // ira mostrar tudo sobre a pizza 
  ...
  try {
    ...
    el.innerHTML = `<div class="empty" style="color:var(--red)">${e.message}</div>`; // Trata erros na carga das pizzas
  }
}

function abrirPizza() { // Função para abrir o modal de criação de pizza
...
function editarPizza(id) { // adiciona ou retira os elementos da pizza 
...
async function salvarPizza() { // Função para salvar uma pizza
...
async function deletarPizza(id, nome) { // Função para deletar uma pizza
...
async function carregarClientes(busca = '') { // Função para carregar os clientes
...
let _t;
function buscarCli(v) { // Função para buscar clientes
  clearTimeout(_t);
  _t = setTimeout(() => carregarClientes(v), 400);
}

function abrirCliente() { // Função para abrir o modal de criação de cliente
...
function editarCliente(id) { // Função para editar um cliente
...
async function deletarCliente(id, nome) { // Função para deletar um cliente
  if (!confirm(`Deletar "${nome}"?`)) return;
  try {
...
async function carregarPedidos() { // Função para carregar os pedidos
...
async function abrirPedido() { // Função para abrir o modal de criação de pedido
  try {
    if (!cPizzas.length)   cPizzas   = await api('GET', '/pizzas');
    if (!cClientes.length) cClientes = await api('GET', '/clientes');
  } catch (e) { toast('Erro ao carregar dados', 'err'); return; }
...
function addItem() { // Função para adicionar um item à lista de itens
  const d = document.createElement('div');
  d.className = 'item-row';
  const opts = cPizzas
...
function recalc() { // Função para recalcular o total do pedido
  let sub = 0;
...
function toggleTroco() { // Função para alternar a exibição do campo de troco
  const pag = document.getElementById('ped-pag').value;
  document.getElementById('wrap-troco').style.display =
    pag === 'dinheiro' ? 'block' : 'none';
}

async function salvarPedido() { // Função para salvar o pedido
...
function abrirStatus(id, status) { // Função para abrir o modal de atualização de status
  document.getElementById('st-id').value  = id;
  document.getElementById('st-val').value = status;
  abrir('m-status');
}

async function salvarStatus() { // Função para salvar o status do pedido
...
async function deletarPedido(id) { // Função para deletar um pedido
...
async function carregarUsuarios() { // Função para carregar os usuários
...
function abrirUsuario() { // Função para abrir o modal de criação de usuário
  ['u-nome','u-email','u-senha'].forEach(id => document.getElementById(id).value = '');
  document.getElementById('u-perfil').value = 'Atendente';
  abrir('m-usuario');
}

async function salvarUsuario() { // Função para salvar um usuário
...
async function deletarUsuario(id, nome) { // Função para deletar um usuário
  if (!confirm(`Deletar "${nome}"?`)) return;
  try {
