...
const SELECT_PEDIDO = ` // Query para selecionar pedidos com informações do cliente
  SELECT
    p.*,
...
function formatarPedido(row, itens = []) { // Função para formatar os dados de um pedido
  if (!row) return null;
  return {
    _id:           row.id,
    id:            row.id,
...
const Pedido = { // Model de Pedido

  async findAll({ garcomId } = {}) { // Função para encontrar todos os pedidos
    await ready;
    let rows;
    if (garcomId) {
  ...
  async findById(id) { // Função para encontrar um pedido pelo ID
    await ready;
  ...
  async create({ clienteId, itens, taxaEntrega = 0, formaPagamento, troco = 0, observacoes = '', mesa = null, origem = 'balcao', garcomId = null }) { // Função para criar um novo pedido
    await ready;
    ...
    for (const item of itens) { // Processa cada item do pedido
      ...
      itensProcessados.push({  // Adiciona o item processado à lista
        pizzaId:       pizza.id,
    ...
    const infoPedido = run(` // Função para criar um novo pedido
      INSERT INTO pedidos
  ...
  async updateStatus(id, status) { // Função para atualizar o status de um pedido
    await ready;
    const info = run(
  ...
  async delete(id) { // Função para excluir um pedido
    await ready;
...
module.exports = Pedido; // Exporta o model de Pedido
